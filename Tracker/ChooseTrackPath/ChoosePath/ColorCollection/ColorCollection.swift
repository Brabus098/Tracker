...
final class ColorCollection: NSObject {
    ...
    private var selectColor: UIColor = .clear
...
extension ColorCollection: UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if selectColor == color {
            ...
            cell.contentView.layer.borderColor = colorArray[indexPath.row].withAlphaComponent(0.3).cgColor
        }
...
extension ColorCollection: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        self.selectColor = colorArray[indexPath.row]
        ...
        if let range = description.range(of: "name = [^;]+", options: .regularExpression) {
            ...
            self.chooseColor = name.filter({$0 != " "}) // сохраняем итоговый результат
        }
...
extension ColorCollection: ColorCollectionProtocol {
    func updateChooseColor(at: String) {
        selectColor = at.toUIColor()
        
        let description = "\(self.selectColor)"
        if let range = description.range(of: "name = [^;]+", options: .regularExpression) {
            let name = String(description[range].dropFirst(6).dropLast(1)).lowercased()
            self.chooseColor = name.filter({$0 != " "}) 
        }
    }
